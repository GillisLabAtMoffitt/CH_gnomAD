...
select_join <- function(gnomad, cosmic){
  ...
  CH_variants <- selected_variants %>%
    ...
    mutate(alt_allele_count_asj = str_match(INFO, "AC_asj=(.*?);")[,2]) %>%
    mutate(alt_allele_count_asj_female = str_match(INFO, "AC_asj_female=(.*?);")[,2]) %>%
    mutate(alt_allele_count_asj_male = str_match(INFO, "AC_asj_male=(.*?);")[,2]) %>%
    
    mutate(alt_allele_count_fin = str_match(INFO, "AC_fin=(.*?);")[,2]) %>%
    mutate(alt_allele_count_fin_female = str_match(INFO, "AC_fin_female=(.*?);")[,2]) %>%
    mutate(alt_allele_count_fin_male = str_match(INFO, "AC_fin_male=(.*?);")[,2]) %>%
    
    mutate(alt_allele_count_oth = str_match(INFO, "AC_oth=(.*?);")[,2]) %>%
    mutate(alt_allele_count_oth_female = str_match(INFO, "AC_oth_female=(.*?);")[,2]) %>%
    mutate(alt_allele_count_oth_male = str_match(INFO, "AC_oth_male=(.*?);")[,2]) %>%
    ...
    mutate(freq_allele_count_asj = str_match(INFO, "AF_asj=(.*?);")[,2]) %>%
    mutate(freq_allele_count_asj_female = str_match(INFO, "AF_asj_female=(.*?);")[,2]) %>%
    mutate(freq_allele_count_asj_male = str_match(INFO, "AF_asj_male=(.*?);")[,2]) %>%
    
    mutate(freq_allele_count_fin = str_match(INFO, "AF_fin=(.*?);")[,2]) %>%
    mutate(freq_allele_count_fin_female = str_match(INFO, "AF_fin_female=(.*?);")[,2]) %>%
    mutate(freq_allele_count_fin_male = str_match(INFO, "AF_fin_male=(.*?);")[,2]) %>%
    
    mutate(freq_allele_count_oth = str_match(INFO, "AF_oth=(.*?);")[,2]) %>%
    mutate(freq_allele_count_oth_female = str_match(INFO, "AF_oth_female=(.*?);")[,2]) %>%
    mutate(freq_allele_count_oth_male = str_match(INFO, "AF_oth_male=(.*?);")[,2]) %>%
    ...
    mutate(nc_alt_allele_count_asj = str_match(INFO, "non_cancer_AC_asj=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_asj_female = str_match(INFO, "non_cancer_AC_asj_female=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_asj_male = str_match(INFO, "non_cancer_AC_asj_male=(.*?);")[,2]) %>%
    
    mutate(nc_alt_allele_count_fin = str_match(INFO, "non_cancer_AC_fin=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_fin_female = str_match(INFO, "non_cancer_AC_fin_female=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_fin_male = str_match(INFO, "non_cancer_AC_fin_male=(.*?);")[,2]) %>%
    
    mutate(nc_alt_allele_count_oth = str_match(INFO, "non_cancer_AC_oth=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_oth_female = str_match(INFO, "non_cancer_AC_oth_female=(.*?);")[,2]) %>%
    mutate(nc_alt_allele_count_oth_male = str_match(INFO, "non_cancer_AC_oth_male=(.*?);")[,2]) %>%
    ...
    mutate(nc_freq_allele_count_asj = str_match(INFO, "non_cancer_AF_asj=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_asj_female = str_match(INFO, "non_cancer_AF_asj_female=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_asj_male = str_match(INFO, "non_cancer_AF_asj_male=(.*?);")[,2]) %>%
    
    mutate(nc_freq_allele_count_fin = str_match(INFO, "non_cancer_AF_fin=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_fin_female = str_match(INFO, "non_cancer_AF_fin_female=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_fin_male = str_match(INFO, "non_cancer_AF_fin_male=(.*?);")[,2]) %>%
    
    mutate(nc_freq_allele_count_oth = str_match(INFO, "non_cancer_AF_oth=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_oth_female = str_match(INFO, "non_cancer_AF_oth_female=(.*?);")[,2]) %>%
    mutate(nc_freq_allele_count_oth_male = str_match(INFO, "non_cancer_AF_oth_male=(.*?);")[,2]) %>%
  ...
  write_delim(CH_variants, paste0(i, "CH_variants.vcf.gz"))
...
for (i in chr_vec){
  ...
  file_list_gnomad <- list.files(path = "/share/lab_gillis/Christelle/gnomAD_skweness/potential_CH_variants/allele_balance/age_distribution/variant_depth/noise_filter/padjust",
                                 pattern = i,
                                 recursive=FALSE,
                                 full.names = TRUE)
