...
# I. Data extraction for variants selection
...
gnomad_decoded <- gnomad %>% 
  ...
  mutate(nbr_individuals_het = rowSums(select(.,`30-35_het`:`>80_het`), na.rm = TRUE)) %>% 
  ...
  add_row("IDs"="All individuals", "ID"="All individuals",
          "age_hist_hom_bin_freq"= "3423|4546|8487|10355|12693|11933|10534|8882|5991|4136",
          "<30_hom" = 2547, ">80_hom" = 1935) %>%
  ...
  # Filter variant found in more than 10 individuals
  mutate(nbr_individuals_hom = rowSums(select(.,`30-35_hom`:`>80_hom`), na.rm = TRUE)) %>%
  
  
  mutate("<30_homhet" = rowSums(select(.,`30-35_het`,`30-35_hom`), na.rm = TRUE),
         "30-35_homhet" = rowSums(select(.,`30-35_het`,`30-35_hom`), na.rm = TRUE),
         "35-40_homhet" = rowSums(select(.,`35-40_het`,`35-40_hom`), na.rm = TRUE),
         "40-45_homhet" = rowSums(select(.,`40-45_het`,`40-45_hom`), na.rm = TRUE),
         "45-50_homhet" = rowSums(select(.,`45-50_het`,`45-50_hom`), na.rm = TRUE),
         "50-55_homhet" = rowSums(select(.,`50-55_het`,`50-55_hom`), na.rm = TRUE),
         "55-60_homhet" = rowSums(select(.,`55-60_het`,`55-60_hom`), na.rm = TRUE),
         "60-65_homhet" = rowSums(select(.,`60-65_het`,`60-65_hom`), na.rm = TRUE),
         "65-70_homhet" = rowSums(select(.,`65-70_het`,`65-70_hom`), na.rm = TRUE),
         "70-75_homhet" = rowSums(select(.,`70-75_het`,`70-75_hom`), na.rm = TRUE),
         "75-80_homhet" = rowSums(select(.,`75-80_het`,`75-80_hom`), na.rm = TRUE),
         ">80_homhet" = rowSums(select(.,`>80_het`,`>80_hom`), na.rm = TRUE)
         ) %>%

  mutate(nbr_individuals = rowSums(select(.,`<30_homhet`:`>80_homhet`), na.rm = TRUE)) %>%
  
  
  
  # Remove variants detected in than 10 individuals
  filter(nbr_individuals_het > 10) %>%
  # Generate AN variable from the INFO var
  mutate(AN = str_match(INFO, "AN=(.*?);")[,2]) %>% 
  mutate(AN_samples_count = (as.numeric(AN) / 2)
    ) %>% 
  mutate(AN_samples_count = coalesce(AN_samples_count, nbr_individuals_het)) %>% 
...
**Should we remove variants tested (AN) in less than 500 samples?**</span>  

```{r, max.print = 30}
head(gnomad_decoded %>% select(IDs, "<30_homhet", "30-35_homhet","35-40_homhet","40-45_homhet","45-50_homhet","50-55_homhet","55-60_homhet","60-65_homhet","65-70_homhet","70-75_homhet","75-80_homhet", ">80_homhet", everything() ))
```
...
gnomad_decoded <- gnomad_decoded %>% 
  ...
  mutate(sample_frequency = sample_count / nbr_individuals_het) %>% 
...
I added a t-test and Mann-Whitney test. For that I used a geom_smooth to get a continuous data reflecting the pattern of the bins distribution. I used a one sided t-test/Mann-Whitney test to look at variants which have greater mean than the reference (shifted to the right - older age)
...
PS : `r  emo::ji("question_mark")` <span style="color:red">**To plot the bins, I am using the number of individuals per age bin divided by the total number of individual with the variant to plot the bins. Should we use instead the number of individuals per age bin divided by the total number of samples (AN)???**</span>  <span style="color:purple">I made the 2 types of histogram</span>  
...
I extracted AN which is the “Total number of alleles in samples”. It is the denominator for allele frequency calculations (AN is allele number), which is 2x the number of individuals in gnomAD with either reference or alternate alleles at the variant site
```{r}
...
In here, I show the distribution of all the variants which as a consequence in DNMT3A gene
...
<!-- # III.Selected variants with Mann-Whitney ≤ 0.05 -->
<!-- ```{r, fig.height= 3, fig.width=7} -->
...
<!-- # V. Extract allele count and frequency variables on the selected variants (code ready for future analysis) -->
<!-- ```{r, print} -->
