...
We probably need to talk to a statistician - maybe Brooke -, I went to the bio-statistic drop off happening on Tuesdays to ask how to do to compare 2 distributions. Jose said to do a chi-square.  
First, using a chi-square test and kolmogorov smirnov test (I added this one on top - found that it is a good one for distribution - but everything comes up as significantly different `r emo::ji("rolling_on_the_floor_laughing")`), I selected the variants with a distribution significantly different to the reference (p ≤ 0.05).  
Then with the skewness test, I wanted to keep the variant as long as its distribution is shifted to the right - older age (skewness test should be > 0 - but not working great as you will see on the plots at the end) so working on that.
```{r}
...
For example, there is multiple type of `Consequence` that we could clean but I don't want to make any mistake.
```{r}
...
```
I only selected the variants not in 3 and 5' for now. Let me know if I am correct and if I can be more selective.  
...
I kept the variant for which the chi square or KS test are significant but you can see that KS is definitely not the best one to choose and the chi square is not what I expected to see...
I am doing some research on that but let me know what you think.
...
I went to the bio stat drop off again but Ram wasn't able to clearly say why the chi square test was not giving the expected result and he didn't really know the skewness test. 
