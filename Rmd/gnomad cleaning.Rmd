...
I filtered out variants that are present in less than or equal to 10 samples.
...
I added a t-test. For that I used a geom_smooth to get a continuous data reflecting the pattern of the bins distribution. I used a one sided t-test to look at variants which have greater mean than the reference (shifted to the right - older age)
...
```{r}
gnomad_decoded1 <- gnomad_decoded1 %>%
  # Generate AN variable from the INFO var
  mutate(samples_count = as.numeric(str_match(INFO, "AN=(.*?);")[,2])) 
```

```{r}
gnomad_decoded1 %>% 
  ggplot(aes(x= samples_count, y= t_test, color = t_test <= 0.05))+
  geom_point(size = 1)+
  ylim(c(0,1))+
  scale_y_log10()+
  theme_minimal()
...
```{r}
gnomad_decoded3 %>% 
  # select(SYMBOL, samples_count) %>% 
  ggplot(aes(x= SYMBOL, y= samples_count))+
  geom_boxplot()
...
# III.Selected variants with t test ≤ 0.05
```{r, fig.height= 3, fig.width=7}
...
# IV. Extract allele count and frequency variables on the selected variants
```{r, print}
