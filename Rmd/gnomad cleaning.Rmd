...
CH_variants %>% select(IDs, HGVSP, everything())
```
We have `r length(unique(CH_variants$IDs))` but with at total of `r length(unique(CH_variants$HGVSP))` HGVSP.
...
# V.Bind with cosmic by : protein change unavailable ~ p.
...
#                   select(-c(AN:t_test)), 
#                 final_cosmic, 
#           by = c("HGVSp" = "HGVSP")) # "IDs", "X.CHROM" = "chr", "POS", "REF", "ALT"
...
<!-- # V. Extract allele count and frequency variables on the selected variants (code ready for future analysis) USING `chr+POS+REF+ALT` binding -->
