...
gnomad_decoded <- gnomad_decoded %>% 
  ...
  mutate(bin_age = case_when(age_bin == "<30" ~ 15, 
...
for(i in unique(data_p$IDs)) {
  ...
  n <- n + 1
  p <- data_plot %>% 
    ggplot(aes(x = bin_age, y= variant_frequency, color= IDs))+
    geom_smooth(alpha= 0.5, se = FALSE, method = "loess",
                span = 0.6, color = "purple"
                )+
    labs(title = n)+
    ylim(0, max(data_plot$variant_frequency))+
    geom_smooth(data = All, aes(x = bin_age, y= variant_frequency, color= IDs), 
                alpha= 0.5, se = FALSE, method = "loess",
                span = 0.6
                )+
    geom_col(data = data_plot ,aes(x= bin_age, y= variant_frequency),
             position = "identity",
             fill = "purple",
             alpha = 0.1)+
    # geom_text(
