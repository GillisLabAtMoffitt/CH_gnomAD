...
In red is the distribution of each variant. In cyan is the reference distribution. I wrote the Mann-Whitney and T test p values of comparing each variant age distribution to the reference population age distribution.
```{r}
...
What are the variable you want in cosmic?
```{r}
cosmic <-
  cosmic %>% 
  ...
  select("Gene.name", gene_name_cosmic, "Accession.Number", IDs, "chr", "POS", REF, "ALT", HGVSG, everything())
...
As ypu can see we have multiple rows for 1 mutation so I calculated the occurrence of each mutation and pivot the data wider to have only 1 mutation per row before to bind with gnomAD.  
But also multiple `HGVSP` for a single `chr, POS, REF, ALT`.   
```{r}
# library(data.table)
# final_cosmic <-
...
final_cosmic <- cosmic %>% 
  group_by(gene_name_cosmic, IDs, chr, POS, REF, ALT) %>% 
  mutate(occurrence_in_cosmic = n())

max_occurence = max(final_cosmic$occurrence_in_cosmic)
final_cosmic <- final_cosmic %>% 
  group_by(gene_name_cosmic, IDs, chr, POS, REF, ALT, HGVSP, occurrence_in_cosmic) %>% 
...
# Bind with cosmic by : chr, POS, REF, ALT
## Data
```{r bind cosmic Ids}
CH_variants <- inner_join(selected_variants %>% 
                  select(-c(AN:t_test)), 
                final_cosmic, 
...
## Plots
```{r}
ids_plot <- paste0(CH_variants$IDs, collapse = "|")
...
# Bind with cosmic by : protein change unavailable ~ p.
...
head(selected_variants %>% select(ens_vep))[1:3,]
```
<!-- <br> -->
...
head(selected_variants %>% select(IDs, ens_vep))
```
...
head(selected_variants %>% select(IDs, "Allele": "LoF_info"))
```

## Extract p. from HGVSp
`r emo::ji("warning")` It is some protein change infp unavailable.
```{r}
...
## data
```{r bind cosmic p}
CH_variants_p <- inner_join(selected_variants %>% 
                  select(-c(AN:t_test)), 
                final_cosmic, 
          by = c("HGVSp" = "HGVSP"))
CH_variants_p
...
## plots
```{r}
ids_plot <- paste0(CH_variants_p$IDs, collapse = "|")

ids_plot <- selected_variants %>%
  filter(str_detect(IDs, ids_plot))
...
<!-- # Bind with cosmic by : p. -->
<!-- For that I need to extract VEP info -->
