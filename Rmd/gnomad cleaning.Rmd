...
final_cosmic1 <- cosmic1 %>% 
  group_by(gene_name_cosmic, IDs, chr, POS, REF, ALT) %>% 
  mutate(occurrence_in_cosmic = n())

max_occurence = max(final_cosmic1$occurrence_in_cosmic)
final_cosmic2 <- final_cosmic1 %>% 
  group_by(gene_name_cosmic, IDs, chr, POS, REF, ALT, occurrence_in_cosmic) %>% 
...
#   unite(IDs, c(chr, POS, REF, ALT), sep = "-", remove = FALSE) %>% 
#   separate(HGVSG, into = c("ENSP", "p_prot_modif"), sep = ":", remove = FALSE) %>% 
#   select("Gene.name", "Accession.Number", IDs, "chr", "POS", REF, "ALT", HGVSG, everything())
```
