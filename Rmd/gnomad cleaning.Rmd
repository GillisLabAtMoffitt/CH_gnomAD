...
gnomad_dnm2 <- gnomad_dnm %>% filter(str_detect(INFO, "Arg882His"))
```
...
<span style="color:red">**I filtered out variants that are present in less than or equal to 10 samples but should we do more?**</span>
...
for(i in unique(data_p$IDs)) {
  ...
  p <- data_plot %>% 
    ...
    geom_text(
      data    = data_plot,
      mapping = aes(x = -Inf, y = -Inf, label = paste0("gene=", gene_in_gnomad)),
      colour = "darkgreen",
      hjust   = -3,
      vjust   = 0
    )+
...
final_cosmic <- final_cosmic %>% ###################################### Clean up here--------------------------------
...
```
`r emo::ji("question_mark")` <span style="color:red">**Should we filter cosmic mutations with a minimum of occurence?**</span>  
`r emo::ji("warning")`But we also have multiple `HGVSP` for a single `chr, POS, REF, ALT`. It can be a problem if we bind by IDs...  
```{r}
(final_cosmic %>% select(IDs, HGVSP, occurrence_in_cosmic, Mutation.somatic.status))[61:67,]
```
...
```{r}
CH_variants %>% 
  select(Primary.site, Primary.histology, Histology.subtype.1,
         HGVSP, Mutation.somatic.status,
         FATHMM.prediction,
         average_age) %>% 
  tbl_summary() %>% 
  bold_labels()
