...
# cosmic1 <- 
  cosmic %>% 
  separate(HGVSG, into = c("chr", "arm", "POS", "ALT"), sep = ":|\\.|>|delins|dup|del", remove = FALSE) %>% 
  select("Gene.name", "Accession.Number", POS) %>% 
  mutate(REF = str_match(POS, "([0-9]*)([a-z]*)"))
  separate(POS, into = c("POS", "REF"), sep = ":|\\.|>|delins|dup|del", remove = FALSE) %>% 
  # separate(Mutation.genome.position, into = c("chr", "start", "end"), sep = ":|-", remove = FALSE) %>% 
  unite(IDs, c(chr, POS, REF, ALT), sep = "-", remove = FALSE) %>% 
  select("Gene.name", "Accession.Number", "chr", "POS", "ALT", everything())
