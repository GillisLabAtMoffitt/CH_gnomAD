...
We want to look at skewness of age distribution in gnomad. We don't have access to real numerical age in this data but was wondering what is the age distribution of Cosmic patients presenting variants found in gnomAD. Note : Each patient is represented once even if patient exhibit multiple variant.  
...
gnomad_variants %>% 
  distinct(Sample.name, .keep_all = TRUE) %>%
  select(Age, SYMBOL) %>%
  mutate(is_CH_gene = case_when(
    SYMBOL %in% known_CH_gene_list     ~ "Yes",
    TRUE                               ~ "No"
  )) %>% 
  tbl_summary(by = is_CH_gene,
...
```
In chromosome `r as.character(mannw_variants %>% distinct(X.CHROM))`, the number of variants with BH ≤ `r pval_selection` is `r nrow(mannw_variants %>% distinct(IDs))`.
```{r Hedges g}
...
CH_variants <- CH_variants %>% 
  mutate(known_CH_genes = case_when(
    SYMBOL %in% known_CH_gene_list                  ~ "known CH gene",
...
CH_variants %>% 
  ...
  ggplot(aes(x= SYMBOL, fill= known_CH_genes))+
  geom_bar()+
...
CH_variants %>%
  ...
  ggplot(aes(x= fct_infreq(SYMBOL), fill= known_CH_genes))+
  geom_bar()+
  ggtitle("Number of variants per gene")+ 
  # scale_fill_discrete(name = NULL, labels = c("Not CH genes", "CH genes"))+
  labs(x= NULL, y="Number of variants")+
  coord_flip()+
  facet_wrap(known_CH_genes ~ ., 
             scales = "free")+ 
  theme(
...
CH_variants %>% 
  ...
  mutate(known_CH_gene = case_when(
    SYMBOL %in% known_CH_gene_list                  ~ SYMBOL,
  ...
  mutate(known_CH_gene = fct_relevel(known_CH_gene, "Not CH genes (average)")) %>%
  ggplot(aes(x= known_CH_gene, y= average_of_variants_per_gene,
             fill= known_CH_gene == "Not CH genes (average)"
             ))+
...
known_CH_variants <- CH_variants %>% 
  filter(SYMBOL %in% known_CH_gene_list) %>% 
  distinct(IDs, .keep_all = TRUE) %>% 
...
variants_in_gnomAD <- CH_variants %>% 
  distinct(IDs, .keep_all = TRUE) 
variants_in_gnomAD %>% 
  select(consequence_group, consequence_rank, known_CH_genes) %>% 
...
variants_in_gnomAD %>% 
  ...
  mutate(name = str_remove(name, "nc_freq_allele_count_"),
    ...
    "overall", "male", "female", "amr", "amr_male", "amr_female", 
...
variants_in_cosmic %>% 
  distinct(Sample.name, .keep_all = TRUE) %>% 
  ggplot(aes(x= Age, fill= known_CH_genes))+
  geom_histogram(binwidth = 1)+
