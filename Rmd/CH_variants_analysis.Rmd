...
```{r Population age}
gnomad_variants %>% 
  ggplot(aes(x= Age, fill= str_detect(Gene.name, known_CH_gene_list)))+
  geom_histogram(binwidth = 1)+
  xlim(0, 100)
gnomad_variants %>% select(IDs, Gene.name, Age)
```
...
CH_variants %>% 
  filter(!is.na(Gene.name) #& ( str_detect(Gene.name, known_CH_genes) | str_detect(Gene.name, "DNMT3|AAK1") )
         ) %>% 
  distinct(IDs, .keep_all = TRUE) %>% 
  ggplot(aes(x= Gene.name, fill= str_detect(Gene.name, known_CH_gene_list)))+
...
CH_variants %>% 
  ...
  labs(x= NULL, y="Number of variants")+
  coord_flip()+
  facet_wrap(known_CH_gene ~ ., scales = "free_y")
...
CH_variants %>% 
  ...
  mutate(known_CH_gene = case_when(
    str_detect(Gene.name, known_CH_gene_list)       ~ Gene.name,
    TRUE                                            ~ "not CH gene"
  ...
  ggtitle("Average number of variants per gene")+
  labs(x= NULL)+
  coord_flip()
