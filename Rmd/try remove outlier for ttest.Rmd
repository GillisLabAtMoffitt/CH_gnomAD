...
for(i in unique(gnomad_decoded$IDs)) {
  ...
  All <- gnomad_decoded %>% filter(IDs == "All individuals"#,
    ...
    # mutate(sample_frequency = case_when( # to set extreme biins to 0
    #   age_bin == "<30" |
    #     age_bin == ">80"                ~ 0,
  ...
  variant$y[variant$x < mean(variant$x ) - (1*sd(variant$x)) | variant$x > mean(variant$x ) + (1*sd(variant$x))] = 0 # set SD to 0
  ...
  ref$y[ref$x < mean(ref$x ) - (1*sd(ref$x)) | ref$x > mean(ref$x ) + (1*sd(ref$x))] = 0
