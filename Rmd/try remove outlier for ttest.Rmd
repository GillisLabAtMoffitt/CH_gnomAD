...
for(i in unique(gnomad_decoded$IDs)) {
  ...
  # Find x for y max
  m <- dat %>% filter(y == max(y, na.rm = TRUE))
  extreme <- dat %>% filter(
    x <= m$x & y > quantile(dat$y, probs = 0.05, na.rm = TRUE) |
      x >= m$x & y < quantile(dat$y, probs = 0.95, na.rm = TRUE)
  )
...
Removing the 5 and 95% sd is not a good solution as it doesn't detect variant 7, 20, ...
