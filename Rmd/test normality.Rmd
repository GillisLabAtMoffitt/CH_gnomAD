...
skew_df <- data.frame(matrix(nrow=1, ncol=0))
...
for(i in unique(gnomad_decoded$IDs)) {
  ...
  # result <- skewness(b$sample_frequency, type = 2)
  # skew <- rbind(skew_df, result)
...
shapiro_df <- shapiro_df %>% `colnames<-`(c("shapiro_test")) %>% 
  mutate(IDs = c(unique(gnomad_decoded$IDs)))
# skew_df <- skew_df %>% `colnames<-`(c("skewness")) %>% 
#   mutate(IDs = c(unique(gnomad_decoded$IDs)))
...
for(i in unique(data_p$IDs)) {
  ...
  p <- data_plot %>% 
    ...
    geom_text(
      data    = data_plot,
      mapping = aes(x = -Inf, y = -Inf, label = paste0("skewness test = ", round(skewness, 3))),
      colour = "darkgreen",
      hjust   = -1.5,
      vjust   = -11
    )+
