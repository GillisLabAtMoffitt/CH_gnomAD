...
gencode <-
  read.delim(#"~/Downloads/gencode.v42.annotation.gtf.gz", 
             paste0(path, "/gnomAD_skweness/data/gencode.v42.annotation.gtf.gz"),
             comment.char="#",
              # sep = " "
...
a %>% 
  ...
  mutate(transcript_type = str_match(additional_information_as_key_value_pairs, 
                                     "transcript_type (.*?);")[,2]) %>% 
  mutate(transcript_name = str_match(additional_information_as_key_value_pairs, 
                                     "transcript_name (.*?);")[,2]) %>% 
  select(chrom, genomic_start_location, genomic_end_location, gene_id, 
         gene_type, gene_name, transcript_type, transcript_name)

DNMT3A_in_gencode <- gencode %>% 
  mutate(gene_in_gencode = str_match(additional_information_as_key_value_pairs, 
                                     "(DNMT3A)(.*?)$")[,2]) %>% 
  filter(gene_in_gencode == "DNMT3A") %>% 
  mutate(chrom = (str_match(chromosome, "chr(.*)")[,2])) %>% 
  mutate(gene_id = str_match(additional_information_as_key_value_pairs, 
                             "gene_id (.*?);")[,2]) %>% 
  mutate(transcript_id = str_match(additional_information_as_key_value_pairs, 
...
```{r}
stat_measure %>% select(IDs, X.CHROM, POS, REF, ALT) %>% 
  inner_join(., DNMT3A_in_gencode,
             by = c("X.CHROM" = "chrom", "POS" = "genomic_start_location"))

stat_measure %>% select(IDs, X.CHROM, POS, REF, ALT) %>% 
  inner_join(., DNMT3A_in_gencode,
             by = c("X.CHROM" = "chrom", "POS" = "genomic_end_location"))
```

```{r}
gnomad %>% select(IDs, X.CHROM, POS, REF, ALT) %>% 
  mutate(X.CHROM = as.character(X.CHROM)) %>% 
  inner_join(., DNMT3A_in_gencode,
             by = c("X.CHROM" = "chrom", "POS" = "genomic_start_location"))
```
